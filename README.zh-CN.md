...
| | |
|---|---|
| Host 半段 | 通过 DSH 的 `shell` 服务执行 git 命令，暴露 `wt.list` / `wt.add` / `wt.remove` / `wt.prune` 四个包私有 RPC 方法；从工作区注册表（当前会话所属工作区）解析主仓库目录。 |
...
- 插件绝不会把 worktree 注册为 DSH 工作区：侧边栏仍按主目录对会话分组。
